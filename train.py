...
def safe_collate(batch):
    """
    Remove samples None ou samples (dict) que têm algum campo None,
    antes de chamar o default_collate.
    """
    # tira elementos completamente None
    ...
    # se for dict, tira qualquer sample que tenha algum valor None
...
class DistributedMusicGenTrainer:
    ...
    def train_process(self, rank: int, world_size: int):
        ...
        # inicializa pesos da parte de vídeo
        ...
        for epoch in range(self.num_epochs):
            ...
            for batch_idx, item in tqdm(enumerate(train_dataloader)):
                # se o batch foi descartado pelo safe_collate
                if item is None:
                    continue
               ...
               # audios = item['audio'].to(rank)
                ...
                all_codes = []
                for prompt, _ in zip(prompts, videos):
                ...
                with torch.autocast(device_type="cuda", dtype=torch.float32):
                    ...
                    codes_single = codes[0]
                    logits = lm_output.logits[0]
                    mask = lm_output.mask[0]
                    
                    codes_oh = self.one_hot_encode(codes_single, num_classes=2048)
                    codes_oh = codes_oh.cuda()
                    logits = logits.cuda()
                    mask = mask.cuda()
                    
                    mask = mask.view(-1)
                    masked_logits = logits.view(-1, 2048)[mask]
                    masked_codes = codes_oh.view(-1, 2048)[mask]
            ...
            with torch.no_grad():
                for val_batch_idx, val_item in tqdm(
                   ...
                   # val_audios = val_item['audio'].to(rank)
                    ...
                    val_all_codes = []
                    for val_prompt, val_video in zip(
                        val_prompts, val_videos
                    ):
                    ...
                    val_attributes, _ = model._prepare_tokens_and_attributes(
                        val_prompts, None
                    )
                    ...
                    with torch.autocast(device_type="cuda", dtype=torch.float32):
                        ...
                        val_codes_single = val_codes[0]
                        val_logits = val_lm_output.logits[0]
                        val_mask = val_lm_output.mask[0]
                        
                        val_codes_oh = self.one_hot_encode(val_codes_single, num_classes=2048)
                        val_codes_oh = val_codes_oh.cuda()
                        val_logits = val_logits.cuda()
                        val_mask = val_mask.cuda()
                        
                        val_mask = val_mask.view(-1)
                        val_masked_logits = val_logits.view(-1, 2048)[val_mask]
                        val_masked_codes = val_codes_oh.view(-1, 2048)[val_mask]
                        
                        val_loss = criterion(val_masked_logits, val_masked_codes)
                        val_losses.append(val_loss.item())

            # se nenhum batch de validação gerou loss, evita divisão por zero
            if len(val_losses) == 0:
                print(
                    f"Epoch {epoch}: nenhum batch de validação gerou loss "
                    f"(todos foram pulados). Pulando validação/early stopping neste epoch."
                )
                continue
