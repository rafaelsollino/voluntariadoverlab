...
from ..modules.adapted_transformer import StreamingTransformer, create_norm_fn
...
class LMModel(StreamingModule):
    ...
    def forward(self, sequence: torch.Tensor, video_features,
        ...
        out = self.transformer(input_, video_features, cross_attention_src=cross_attention_input,
    ...
    def compute_predictions(
            self, codes: torch.Tensor,video_features,
            conditions: tp.List[ConditioningAttributes],
            condition_tensors: tp.Optional[ConditionTensors] = None,
            stage: int = -1,
        ...
        logits = model(sequence_codes, video_features, conditions, condition_tensors, stage=stage)  # [B, K, S, card]
    ...
    def prepare_inputs_for_generation(self, *args, **kwargs):
        # Implement a dummy method to satisfy PEFT's requirements
        return None
    def _sample_next_token(self,
        ...
        if two_step_cfg and cfg_conditions != {}:
            ...
            cond_logits = model(sequence, video_features, conditions=[], condition_tensors=condition_tensors)
            state = self.get_streaming_state()
            self.set_streaming_state(unconditional_state)
            uncond_logits = model(sequence, video_features, conditions=[], condition_tensors=null_condition_tensors)
            ...
            if condition_tensors:
                ...
                video_features = torch.cat([video_features, video_features], dim=0)
        ...
        with self.streaming():
            ...
            for offset in range(start_offset_sequence, gen_sequence_len):
                ...
                next_token = self._sample_next_token(
                    curr_sequence, video_features, cfg_conditions, unconditional_state, use_sampling, temp, top_k, top_p,
