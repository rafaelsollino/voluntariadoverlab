...
class LMModel(StreamingModule):
    ...
    def generate(self,
        ...
        with self.streaming():
            ...
            #print("gen_sequence_len", gen_sequence_len)
            
            for offset in range(start_offset_sequence, gen_sequence_len):
                # get current sequence (note that the streaming API is providing the caching over previous offsets)
                #print(prev_offset, offset, "**")
                ...
                #print("the shape of next_token is", next_token.shape)
...
class LMFeatureExtractor(StreamingModule):
    """Transformer-based language model on multiple streams of codes.
    ...
    def forward(self, sequence: torch.Tensor,
        ...
        z_in = self.transformer(input_, cross_attention_src=cross_attention_input,
                               src_mask=(self.attn_mask_per_stage[stage] if stage >= 0 else None))
        if self.out_norm:
            z_in = self.out_norm(z_in)
        return z_in
    def compute_predictions(
            self, codes: torch.Tensor,
            conditions: tp.List[ConditioningAttributes],
