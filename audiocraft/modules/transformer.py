...
class StreamingTransformerLayer(nn.TransformerEncoderLayer):
    ...
    def _cross_attention_block(self, src: torch.Tensor,
                               cross_attention_src: torch.Tensor, is_generating) -> torch.Tensor:
    ...
    def forward(self, src: torch.Tensor, src_mask: tp.Optional[torch.Tensor] = None,  # type: ignore
        ...
        ###
        is_generating = cross_attention_src is not None and src_mask is not None
        ###
            ...
            if cross_attention_src is not None:
                x = x + self.layer_scale_cross(
                    self._cross_attention_block(
                        self.norm_cross(x), cross_attention_src, is_generating=is_generating))
...
class StreamingTransformer(StreamingModule):
    ...
    def forward(self, x: torch.Tensor, *args, **kwargs):
        ...
        for layer in self.layers:
            x, is_generating = self._apply_layer(layer, x, *args, **kwargs)
