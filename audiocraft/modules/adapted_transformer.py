...
class VideoMusicAttention(nn.Module):
    def __init__(self, music_dim=1536, vivit_dim=768,num_heads=8):
        ...
        self.W_q = nn.Linear(music_dim, music_dim)
        self.W_k = nn.Linear(vivit_dim, music_dim)
        self.W_v = nn.Linear(vivit_dim, music_dim)

        self.W_o = nn.Linear(music_dim, music_dim)

        self.scale = self.head_dim ** - 0.5
    
    def forward(self, z_in, video_features):
        """
        z_in : Music features [batch_size, music_seq_len, music_dim]
        video_features: ViViT features [batch_size, video_seq_len, vivit_dim]
        """


        B, T_m, _ = z_in.shape
        B2, T_v, _ = video_features.shape
        Q = self.W_q(z_in).view(B, T_m, self.num_heads, self.head_dim)
        K = self.W_k(video_features).view(B2, T_v, self.num_heads, self.head_dim)
        V = self.W_v(video_features).view(B2, T_v, self.num_heads, self.head_dim)
        ...
        out = out.transpose(1,2).contiguous().view(B, T_m, -1)
...
class StreamingMultiheadAttention(StreamingModule):
    ...
    def forward(self, query: torch.Tensor, key: torch.Tensor, value: torch.Tensor,
                ...
                average_attn_weights=True, is_causal=False, is_generating = False):
...
class StreamingMultiheadVideoAttention(StreamingModule):
    """Similar to `nn.MultiheadAttention` but with support for streaming, causal evaluation.
...
class StreamingTransformerLayer(nn.TransformerEncoderLayer):
    ...
    def __init__(self, d_model: int, num_heads: int, dim_feedforward: int = 2048, dropout: float = 0.1,
        ...
        if cross_attention:
            ...
            self.video_attention = StreamingMultiheadVideoAttention(
                cross_attention=True, qk_layer_norm=qk_layer_norm_cross, 
                **attn_kwargs, **factory_kwargs)
            # Norm and dropout
        ...
        #    vivit_dim=768
        #)
        #self.video_attention_scale = nn.Parameter(torch.rand(1)*0.02)
        self.video_attention_scale  = nn.Parameter(torch.tensor([0.0]), requires_grad=True)
    ...
    def _cross_attention_block(self, src: torch.Tensor,
        ...
        x = self.cross_attention(
            src, cross_attention_src, cross_attention_src, need_weights=False, is_generating=True)[0]
        
        return self.dropout_cross(x)  # type: ignore
    def _video_attention_block(self, video_features, cross_attention_src):
        x = self.video_attention(
            video_features, cross_attention_src, cross_attention_src, need_weights=False, is_generating=True)[0]
        
        return self.dropout_cross(x)    
    def forward(self, src: torch.Tensor, video_features, src_mask: tp.Optional[torch.Tensor] = None,  # type: ignore
        ...
        if self.norm_first:
            ...
            if cross_attention_src is not None:
                ...
                video_features = self.video_projection_layer(video_features)
                original_cross_attention = self._cross_attention_block(self.norm_cross(x), cross_attention_src)
                
                video_cross_attention = self.video_attention_scale * self._video_attention_block(self.norm_cross(x), video_features) 
                combined_cross_attention = original_cross_attention + video_cross_attention
                
                x = x + self.layer_scale_cross(
                    combined_cross_attention
                )
...
class StreamingDecoupledTransformer(nn.Module):
    """Transformer with decoupled cross-attention for text and audio."""
    def __init__(self, d_model: int, num_heads: int, num_layers: int,
                 ...
                 positional_embedding: str = 'sin', max_period: float = 10_000,
                 positional_scale: float = 1., device=None, dtype=None, **kwargs):
        super().__init__()
        ...
        self.layers = nn.ModuleList([
            StreamingDecoupledTransformerLayer(
                d_model=d_model,
                num_heads=num_heads,
    ...
    def forward(self, x: torch.Tensor, 
                video_context: tp.Optional[torch.Tensor], *args, **kwargs):
        B, T, C = x.shape
        text_context = x
        # Handle streaming offsets
        if 'offsets' in self._streaming_state:
            offsets = self._streaming_state['offsets']
        else:
            offsets = torch.zeros(B, dtype=torch.long, device=x.device)
        ...
        for layer in self.layers:
            x = layer(x, text_context, video_context)
...
def create_sin_embedding(positions: torch.Tensor, dim: int, max_period: float, dtype: torch.dtype):
    """Create sinusoidal positional embeddings."""
    half = dim // 2
    positions = positions.float()
    freqs = torch.exp(-math.log(max_period) * torch.arange(start=0, end=half, dtype=torch.float32) / half).to(device=positions.device)
